"""
UI + application logic for Laptop Health (Linux).

Security hardening notes:
- All external commands go through system.run_cmd() or system.run_privileged()
- No shell=True usage
- Executables resolved via shutil.which inside system wrappers
"""
    ...
    def run(self) -> None:
        try:
            with tempfile.TemporaryDirectory(prefix="laptophealth_") as td:
                ...
                MAX_REPORT_BYTES = 5 * 1024 * 1024  # 5 MiB safety cap

                if os.path.exists(csv_path):
                    with open(csv_path, "r", encoding="utf-8", errors="replace") as f:
                        csv_text = f.read(MAX_REPORT_BYTES + 1)
                    if len(csv_text) > MAX_REPORT_BYTES:
                        csv_text = csv_text[:MAX_REPORT_BYTES] + "\n…(truncated powertop CSV)…"

                if os.path.exists(html_path):
                    with open(html_path, "r", encoding="utf-8", errors="replace") as f:
                        html_text = f.read(MAX_REPORT_BYTES + 1)
                    if len(html_text) > MAX_REPORT_BYTES:
                        html_text = html_text[:MAX_REPORT_BYTES] + "\n…(truncated powertop HTML)…"
            rows = []
            if csv_text:
...
"""
    ...
    def investigate_wakeups(self):
        ...
        if getattr(self, "_wakeup_worker", None) is not None and self._wakeup_worker.isRunning():
            QtWidgets.QMessageBox.information(self, "Investigate wakeups", "Wakeup investigation is already running.")
            return

        # Clear privilege boundary: powertop needs admin for reliable wakeup stats
        resp = QtWidgets.QMessageBox.question(
            self,
            "Investigate wakeups",
            "This will run powertop with administrator privileges (pkexec/sudo) to collect wakeup statistics.\n\nProceed?",
    ...
    def refresh_fast(self):
        ...
        cpu_t = _to_float(temps_s.get("cpu"))
        gpu_t_s = _to_float(temps_s.get("gpu"))
        ssd_t_s = _to_float(temps_s.get("ssd"))

        nvgpu, _ = sensors.read_nvidia_gpu_temp()
        nvme_t, _ = sensors.read_nvme_temp()

        nvgpu_f = _to_float(nvgpu)
        nvme_f = _to_float(nvme_t)

        gpu_t = nvgpu_f if nvgpu_f is not None else gpu_t_s
        ssd_t = nvme_f if nvme_f is not None else ssd_t_s
